    ...
    <main className="dashboard-page">
      <div className="dashboard-shell">
        <aside className="dashboard-sidebar">
          <div className="sidebar-user"><div className="user-avatar">{initials(profile.name)}</div><div><strong>{profile.name}</strong><span>Buyer account</span></div></div>
        ...
        <section className="dashboard-content">
          ...
          {activeTab === 'profile' && <Profile profile={profile} setProfile={setProfile} saved={saved} error={profileError} onSave={handleSave} />}
        </section>
      </div>
    </main>
  );
...
function Profile({ profile, setProfile, saved, error, onSave }) {
  return <><header className="dashboard-header"><div><p className="eyebrow">Account</p><h1>Profile settings</h1><p>Update the personal details used for order notifications.</p></div></header><section className="dashboard-panel profile-panel"><div className="profile-intro"><div className="large-user-avatar">{initials(profile.name)}</div><div><h2>{profile.name}</h2><p>Authenticated buyer account</p></div><button type="button" disabled><MdEdit /> Avatar soon</button></div><div className="profile-fields"><label>Full name<input value={profile.name} onChange={(event) => setProfile({ ...profile, name: event.target.value })} /></label><label>Username<input value={profile.username} onChange={(event) => setProfile({ ...profile, username: event.target.value })} /></label><label className="full-field">Email address<input type="email" value={profile.email} readOnly /><small>Email changes will be added with verified email flows.</small></label></div><div className="profile-save-row"><span>{saved && <><MdCheckCircle /> Changes saved</>}{error && <b className="profile-error">{error}</b>}</span><button type="button" onClick={onSave}><MdSave /> Save changes</button></div></section></>;
}
